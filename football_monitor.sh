...
while IFS=',' read -r match_id home_team away_team league minute_num; do
    ...
    # Create structured message format optimized for Telegram and Claude input
    MESSAGE="🏆 MATCH ALERT 🏆
League: $league
Teams: ${HOME_TEAM_NAME:-$home_team} vs ${AWAY_TEAM_NAME:-$away_team}
Minute: ${minute_num}'
Score: ${HOME_GOALS_INT}-${AWAY_GOALS_INT}

📊 STATS:
Shots_On: ${ON_TARGET_HOME:-0}-${ON_TARGET_AWAY:-0}
Shots_Off: ${OFF_TARGET_HOME:-0}-${OFF_TARGET_AWAY:-0}
Attacks: ${ATTACKS_HOME:-0}-${ATTACKS_AWAY:-0}
Dangerous_Attacks: ${DANGEROUS_ATTACKS_HOME:-0}-${DANGEROUS_ATTACKS_AWAY:-0}
